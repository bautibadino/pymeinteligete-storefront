  ...
  if (!specifications || specifications.length === 0) {
    return (
      <div className={productDetailInnerPanelClassName("px-4 py-4")}>
        <p className="text-sm leading-7 text-muted-foreground xl:text-white/66">
    ...
    <dl className="grid gap-3 sm:grid-cols-2">
      {specifications.map((spec, index) => (
        ...
        >
          ...
          <dd className="mt-1 text-sm font-medium leading-6 text-foreground xl:text-white/84">
            {spec.value}
          </dd>
        </div>
      ))}
  ...
  return (
    <div className={cn("grid gap-2 sm:gap-3 md:gap-4", className)}>
      ...
      >
        ...
        <div className="flex h-full min-h-0 items-center justify-center bg-white xl:min-h-[580px] xl:bg-transparent">
          {currentImage ? (
            <img
            ...
            <div className="grid h-full w-full place-items-center text-center">
              <div className="grid gap-3">
                <div className="mx-auto rounded-full border border-black/10 bg-black/[0.03] p-4 text-muted-foreground xl:border-white/10 xl:bg-white/[0.05] xl:text-white/58">
                  <ImageOff className="size-7" aria-hidden="true" />
                </div>
                <div>
                  <p className="font-heading text-xl font-semibold text-foreground md:text-2xl xl:text-white">
                    Imagen pendiente
                  </p>
                  <p className="mt-2 text-sm leading-6 text-muted-foreground xl:text-white/66">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? "border-black/12 bg-white shadow-[0_14px_28px_rgba(15,23,42,0.14)] xl:border-white/70 xl:bg-white/[0.08] xl:shadow-[0_14px_28px_rgba(0,0,0,0.22)]"
                    : "border-black/10 bg-white/72 opacity-80 hover:opacity-100 xl:border-white/10 xl:bg-white/[0.03] xl:opacity-70",
                )}
              >
                <img
                  src={image.url}
                ...
                <span
                  className={cn(
                    ...
                    isActive ? "border-black/8 xl:border-white/30" : "border-transparent",
                  )}
                />
              </button>
            );
...
export function ProductDetailSegmentedTabs({
  ...
  return (
    <div className={productDetailCardClassName(cn("grid gap-4 p-4 md:gap-5 md:p-6", className))}>
      <div
        className="flex gap-2 overflow-x-auto rounded-full border border-black/10 bg-black/[0.03] p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.65)] xl:border-white/10 xl:bg-black/20 xl:backdrop-blur-md"
        role="tablist"
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isActive
                  ? "bg-foreground text-background shadow-[0_10px_24px_rgba(15,23,42,0.12)] xl:bg-white xl:text-foreground xl:shadow-[0_10px_24px_rgba(0,0,0,0.18)]"
                  : "text-muted-foreground hover:text-foreground xl:text-white/62 xl:hover:text-white",
              )}
            >
              {section.label}
            </button>
      ...
      >
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground xl:text-white/44">
            {activeSection.eyebrow}
          </p>
          <div className="mt-3 flex items-start justify-between gap-4">
            <h2 className="font-heading text-3xl font-semibold tracking-[-0.04em] text-foreground md:text-4xl xl:text-white">
              {activeSection.title}
            </h2>
            <ChevronRight className="mt-1 hidden size-5 shrink-0 text-muted-foreground md:block xl:text-white/28" aria-hidden="true" />
          </div>
        </div>
        ...
        <div className="grid gap-4">
          {activeSection.paragraphs.map((paragraph, index) => (
            <p
              ...
              className="text-sm leading-7 text-muted-foreground md:text-base xl:text-white/72"
            >
              {paragraph}
            </p>
          ))}
            ...
            {activeSection.notes.map((note, index) => (
              <li
                ...
                className={productDetailInnerPanelClassName(
                  "px-4 py-3 text-sm leading-6 text-muted-foreground xl:text-white/72",
                )}
              >
                {note}
              </li>

import Link from "next/link";
import type { Route } from "next";
import { Layers } from "lucide-react";

import type { CategoryTileModule } from "@/lib/modules/category-tile";

/**
 * Category Tile — Masonry.
 *
 * Layout asimétrico: primer tile grande (hero tile) y los restantes
 * más pequeños. Impacto visual fuerte. Cuando hay menos de 3 tiles,
 * todos son iguales. Consume exclusivamente CSS vars del sistema de
 * design tokens.
 */
export function CategoryTileMasonry({ module }: { module: CategoryTileModule }) {
  const { title, subtitle, tiles } = module;

  const [heroTile, ...restTiles] = tiles;

  return (
    <section
      aria-labelledby={title ? `cat-tile-${module.id}-title` : undefined}
      className="py-10"
      data-template="category-tile-masonry"
    >
      {(title || subtitle) ? (
        <div className="mb-6">
          {title ? (
            <h2
              id={`cat-tile-${module.id}-title`}
              className="font-heading text-2xl font-semibold text-foreground md:text-3xl"
            >
              {title}
            </h2>
          ) : null}
          {subtitle ? (
            <p className="mt-1 text-sm text-muted md:text-base">{subtitle}</p>
          ) : null}
        </div>
      ) : null}

      {tiles.length < 3 ? (
        <ul className="grid grid-cols-2 gap-3" role="list">
          {tiles.map((tile) => (
            <li key={tile.href}>
              <TileCard tile={tile} moduleId={module.id} className="aspect-square" />
            </li>
          ))}
        </ul>
      ) : (
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
          {heroTile ? (
            <div className="col-span-2 md:col-span-1 md:row-span-2">
              <TileCard
                tile={heroTile}
                moduleId={module.id}
                className="aspect-video md:h-full md:aspect-auto"
              />
            </div>
          ) : null}

          <ul className="contents" role="list">
            {restTiles.map((tile) => (
              <li key={tile.href}>
                <TileCard tile={tile} moduleId={module.id} className="aspect-square" />
              </li>
            ))}
          </ul>
        </div>
      )}
    </section>
  );
}

function TileCard({
  tile,
  moduleId,
  className,
}: {
  tile: CategoryTileModule["tiles"][number];
  moduleId: string;
  className?: string;
}) {
  return (
    <Link
      href={tile.href as Route}
      aria-label={tile.label}
      className={[
        "group relative block w-full overflow-hidden rounded-xl bg-panel-strong shadow-tenant",
        "transition-transform duration-200 hover:scale-[1.02]",
        "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary",
        className ?? "",
      ].join(" ")}
    >
      {tile.imageUrl ? (
        <img
          src={tile.imageUrl}
          alt={tile.label}
          className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105"
          loading="lazy"
        />
      ) : (
        <div
          aria-hidden="true"
          className="flex h-full w-full items-center justify-center bg-gradient-to-br from-accent-soft to-primary-soft"
        >
          <Layers
            className="size-12 text-primary opacity-60"
            aria-hidden="true"
          />
        </div>
      )}

      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-ink/80 to-transparent p-3">
        <span className="font-heading text-sm font-semibold leading-tight text-white md:text-base">
          {tile.label}
        </span>
      </div>
    </Link>
  );
}

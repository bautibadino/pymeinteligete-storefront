...
export async function POST(request: Request) {
  ...
  if (!runtime.apiBaseUrl) {
    return NextResponse.json(
      {
        error: "No se pudo resolver la API base de la plataforma para cotizar envíos.",
  ...
  try {
    ...
    return NextResponse.json(
      {
        error: "Body JSON inválido para cotizar envíos.",
  ...
  const upstreamUrl = new URL(STOREFRONT_API_PATHS.shippingQuote, `${runtime.apiBaseUrl}/`);
  ...
  try {
    ...
    return NextResponse.json(
      {
        error: "No se pudo cotizar el envío para este storefront.",

...
import {
  STOREFRONT_API_PATHS,
  STOREFRONT_INTERNAL_ERROR_CODES,
} from "@/lib/contracts/storefront-v1";
...
export async function POST(request: Request) {
  ...
  if (!runtime.apiBaseUrl) {
    return NextResponse.json(
      {
        error: "No se pudo resolver la API base de la plataforma para validar el carrito.",
  ...
  try {
    ...
    return NextResponse.json(
      {
        error: "Body JSON inválido para validar el carrito.",
  ...
  const upstreamUrl = new URL(STOREFRONT_API_PATHS.cartValidate, `${runtime.apiBaseUrl}/`);
  ...
  try {
    ...
    return NextResponse.json(
      {
        error: "No se pudo validar el carrito para este storefront.",
